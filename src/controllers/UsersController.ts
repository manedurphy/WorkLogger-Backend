...
import { controller, httpGet, request, response, httpPost, BaseHttpController } from 'inversify-express-utils';
...
export class UsersController extends BaseHttpController {
    ...
    private async Register(@request() req: Request, @response() res: Response) {
        ...
        try {
            ...
            if (process.env.REGISTER !== 'testing') this.authService.sendVerificationEmail(activationPassword); // For testing only. Remove for production.
    ...
    private async Login(@request() req: Request, @response() res: Response) {
        try {
            ...
            if (process.env.LOGIN === 'testing') await existingUser.update({ active: true }); // For testing only. Remove for production.
    ...
    private async VerifyToken(@request() req: AuthenticatedRequest, @response() res: Response) {
        try {
            return this.ok(this.userService.GetTokenValidResponse(req.payload.userInfo));
        } catch (error) {
    ...
    @httpGet('/refresh')
    private async GetNewToken(@request() req: AuthenticatedRequest, @response() res: Response) {
        try {
            const token = this.authService.GenerateToken(req.payload.userInfo);
            const refreshToken = this.authService.GenerateRefreshToken(req.payload.userInfo);
